    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::InvalidIpv4TotalLength {
                ...
                f,
                "IPv4 total_len {total_len} is smaller than header_len {header_len}"
            ),
            Self::FragmentedPacket => write!(f, "packet is a fragment — payload is incomplete"),
            Self::InvalidChecksum { expected, actual } => write!(
                f,
                "invalid checksum: expected {expected:#06x}, got {actual:#06x}"
            ),
            Self::InvalidUdpLength { header_len, actual } => write!(
                f,
                "UDP length field {actual} is smaller than header length {header_len}"
            ),
            Self::InvalidTcpOptionLength { kind, length } => write!(
                f,
                "TCP option kind {kind} has invalid length {length} (minimum 2)"
            ),
